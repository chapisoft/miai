...
async def extract_ocr(
    ...
    return ApiResponse.success(data=result, message="Bóc tách văn bản OCR thành công")
...
async def extract_identity_card(
    ...
    return ApiResponse.success(data=result, message="Bóc tách giấy tờ tùy thân thành công")
...
async def extract_vat_invoice(
    ...
    return ApiResponse.success(data=parsed_invoice, message="Bóc tách hóa đơn VAT thành công")


@router.post("/fdi/extract", response_model=ApiResponse[FdiExtractionResponse], summary="Bóc tách biểu mẫu FDI Form Station rảnh tay")
@router.post("/fdi-form", response_model=ApiResponse[FdiExtractionResponse], summary="Bóc tách biểu mẫu FDI")
async def extract_fdi_form(
    template_code: str = "FDI_INBOUND_DELIVERY_V1",
    current_user: dict = Depends(get_current_user)
) -> ApiResponse[FdiExtractionResponse]:
    ...
    extracted_data = RoiExtractor.extract_from_mock_image(
        template_code=template_code,
        detected_text_map=mock_detected_text,
        execution_time_ms=115.0
    )
    result = FdiExtractionResponse(
        template_code=template_code,
        document_code="DOC-FDI-9901",
        extracted_fields=extracted_data.get("fields", {}),
        confidence_scores={k: 0.98 for k in extracted_data.get("fields", {}).keys()},
        has_signature=True,
        processing_time_ms=115.0
    )
    return ApiResponse.success(data=result, message="Bóc tách biểu mẫu FDI thành công")
...
async def extract_boq_table(
    ...
    return ApiResponse.success(data=parsed_boq, message="Bóc tách bảng BoQ thành công")
